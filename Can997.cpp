...
INT8U CAN_SendMsg(INT32U id, INT8U ext,INT8U len, INT8U* buf) {
...
INT8U CAN_BeginSlave() {
	...
	//CAN0.init_Filt(4, 0, ((long)CAN_ANTRIEB242) << 16);
...
INT8U  getCANDataPrivate(long duration, EngineMsmtU& Engine) {
	...
	long start = millis();
#ifdef DEBUGSERIAL
	DEBUGSERIAL.println("getCANDataPrivate");
#endif
...
INT8U sendBothPrivateCan(EngineMsmtU& _Engine) {
	...
	sndStat = CAN_SendMsg(CAN_PRIVATE1,ext, 8, (byte*)&_Engine.b[0]);
	if (sndStat != CAN_OK) return sndStat;
	ext = 0;
	if (CAN_PRIVATE1 > 0x7FF) ext = 1;
	sndStat = CAN_SendMsg(CAN_PRIVATE2,ext, 8, (byte*)&_Engine.b[8]);
	return sndStat;
}
